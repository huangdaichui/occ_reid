...
class ReIDEvaluator:
    ...
    def __init__(self, dist, mode):
        assert dist in ['cosine', 'euclidean', 'cosine_part']
        self.dist = dist
        assert mode in ['inter-camera', 'intra-camera', 'all']
        self.mode = mode

    def evaluate(self, query_features, query_camids, query_pids, gallery_features, gallery_camids, gallery_pids, query_local_features, gallery_local_features, query_confidence, gallery_confidence):

        '''compute distance matrix'''
        if self.dist is 'cosine_part':
            scores = self.cosine_dist(query_features, gallery_features)
            weighted_local_dist, confidence_sum_g = self.compute_weighted_dist(query_local_features, gallery_local_features, query_confidence, gallery_confidence, threshold=0.15)
            scores = weighted_local_dist + scores
            scores = np.true_divide(scores, confidence_sum_g)
    ...
    def compute_weighted_dist(self, x, y, x_confid, y_confid, threshold):
        '''compute dist between local features and give them weights based on keyppoints confidence'''

        for i in range(13):
            local_non_weights = self.cosine_dist(x[:, :, i], y[:, :, i])
            qurey_local_confidence = np.where(x_confid[:, i][:, np.newaxis] > threshold, x_confid[:, i][:, np.newaxis], 0)
            qurey_local_confidence = np.where(qurey_local_confidence <= threshold, qurey_local_confidence, 1)
            gallery_local_confidence = np.where(y_confid[:, i][np.newaxis, :] <= threshold, y_confid[:, i][np.newaxis, :], 1)
            gallery_local_confidence = np.where(gallery_local_confidence > threshold, gallery_local_confidence, 0)
            local_confid = np.matmul(qurey_local_confidence, gallery_local_confidence)
            local_item = np.multiply(local_non_weights, local_confid)
            if i == 0:
                local_item_sum = local_item
                confidence_sum = local_confid
            else:
                local_item_sum = local_item + local_item_sum
                confidence_sum = local_confid + confidence_sum
        confidence_sum_g = np.ones(local_confid.shape) + confidence_sum
        # for i in range(13):
        #     local_non_weights = self.cosine_dist(x[:, :, i], y[:, :, i])
        #     local_confid = np.sqrt(np.matmul(x_confid[:, i][:, np.newaxis], y_confid[:, i][np.newaxis, :]))
        #     local_item = np.multiply(local_non_weights, local_confid)
        #     if i == 0:
        #         local_item_sum = local_item
        #         confidence_sum = local_confid
        ...
        return local_item_sum, confidence_sum_g
