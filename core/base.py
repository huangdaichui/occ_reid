...
class Base:
	...
	def __init__(self, config):
		...
		self.train_dataset = str(config.train_dataset)
		self.train_dataset = self.train_dataset.replace('[', '').replace(']', '').replace('\'', '')
	...
	def _init_creiteron(self):
		...
		self.L2_creiteron = nn.MSELoss()
		self.local_id_creiteron_list = []
		for i in range(13):
			self.local_id_creiteron = CrossEntropyLabelSmooth(self.pid_num)
			self.local_id_creiteron_list.append(self.local_id_creiteron)
	...
	def save_model(self, save_epoch):
		...
		# save model
		output_path = self.output_path + self.train_dataset + 'l{}m{}c{}'.format(self.config.local_loss, self.config.map_loss, self.config.ch_loss)
		if os.path.exists(output_path):
			print(output_path)
		else:
			os.makedirs(output_path)

		file_path = os.path.join(output_path, 'model_{}.pkl'.format(save_epoch))
		...
		if self.max_save_model_num > 0:
			...
			root, _, files = os_walk(output_path)
			for file in files:
				if '.pkl' not in file:
					files.remove(file)
			# remove extra model
	...
	def resume_model(self, resume_epoch):
		...
		output_path = self.output_path + self.train_dataset + 'l{}m{}c{}'.format(self.config.local_loss, self.config.map_loss, self.config.ch_loss)
		model_path = os.path.join(output_path, 'model_{}.pkl'.format(resume_epoch))
